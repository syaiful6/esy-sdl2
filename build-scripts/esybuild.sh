...
setup_unix() {
  CONFIG_SHELL=bash CFLAGS="-fPIC -fcommon" LDFLAGS="-L/usr/local/lib" "${SOURCE_PATH}"/configure \
    --prefix="$INSTALL_PREFIX"
}

make_and_install_unix() {
    gmake && gmake install
}
...
while [ "$1" != "" ]; do
  case $1 in
    ...
    --os=windows)
      ...
      ;;
    --os=unix)
       execute_inside_build_dir setup_unix
       execute_inside_build_dir make_and_install_unix
       break
       ;;
    *)
      break
